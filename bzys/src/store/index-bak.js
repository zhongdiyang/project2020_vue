...
const store = new Vuex.Store({
   ...
   */
    ...
    */
        solar2lunar: function (y1, m1, d1) { // 参数区间1900.1.31~2100.12.31
          if (y1 < 1900 || y1 > 2100) { return -1 }// 年份限定、上限
          if (y1 === 1900 && m1 === 1 && d1 < 31) { return -1 }// 下限
          var objDate
          if (!y1) { // 未传参  获得当天
            objDate = new Date()
          } else {
            objDate = new Date(y1, parseInt(m1) - 1, d1)
          }
          var i
          var leap = 0
          var temp = 0
    ...
    */
      ...
      function wxbz (year, birthTime) {
        ...
        var hSection = StemsBranch.getHourStems(dayStems, h) + StemsBranch.getHourBranch(h)
        var wx = {}
      ...
      let wb = wxbz(value.date, value.hour)
      state.formInfo.birth_date = value.date
      state.formInfo.birth_hour = value.hour
      state.formInfo.bz = wb.bz
      state.formInfo.wx = wb.wx
      state.formInfo.birth_date_lunar = wb.lunar
    },
